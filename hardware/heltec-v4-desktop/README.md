# Heltec V4 Desktop Node — Build Guide

**Version:** v1.1.0
**Board:** Heltec WiFi LoRa 32 V4
**Status:** 🚧 In Progress

A simple indoor Meshtastic node designed to sit on a desk. Shares the same body as the V3 build — only the tray and lid differ to fit the V4's updated board layout.
...
- Button access holes — the V4 has RST and PRG buttons; rarely needed in normal operation, so instead of plungers there are small holes you can poke through with a pin, toothpick, or pen
...
| Part | Material | Notes |
|---|---|---|
| `body` | PETG | Same body as V3 — print once, works for both |
| `tray-v4` | PETG | Board cradle specific to Heltec V4 |
| `lid` | PETG | Top cover |
...
7. Slide the Heltec V4 into the tray; it should sit flush
...
Basic setup:
- Flash Meshtastic firmware for Heltec WiFi LoRa 32 V4 from [meshtastic.org/downloads](https://meshtastic.org/downloads)
